    ...
    if (!empty($_SESSION['auth']) and $_SESSION['status'] == 'admin' ) {
      // сделать страницу всех тем и записей в каждом пользователе отдельно
        ...
        function showUsersTable($link)
        {
          ...
          $query = "SELECT users.id as id, users.login as login, statuses.name as status, users.banned as banned
          ...
          foreach ($data as $user) {
                  $content .= " <tr>
                    ...
                    $content .= "<td><a href=\"/admin/notes.php?id={$user['id']}\"> more notes </a></td>
                  </tr>" ;
          }
          $content .= '</table>' ;
